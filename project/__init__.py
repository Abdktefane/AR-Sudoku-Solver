...
def find_grid_with_contours(src, edited):
    image = edited.copy()
    ...
    if len(contours) != 0:
        ...
        rect = 0
        if len(cnt) == 4:
            image, rect, src, M = prespective_transformation(src, cnt)
...
def prespective_transformation(img, cnt):
...
def pre_processing(src):
    ...
    grid, src = find_grid_with_contours(src, inverse_binary_image)
